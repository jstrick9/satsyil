...
# MAGIC - **Created on:** 2026-07-08
# MAGIC - **Last updated by:** Drew McPherson
# MAGIC - **Last updated on:** 2026-07-08
# MAGIC
# MAGIC ## Changelog
# MAGIC - **2026-07-08 (Drew McPherson):** Initial notebook deployment
...
# ────────────────────────────────────────────────────────────────────────────
# The same table is used for both training and prediction. For training, it is
# necessary to restrict the training set to records that are old enough to have
# registered or abandoned. Thus for training we compute the start of the fiscal
# year that is at least 72 months old.
# ────────────────────────────────────────────────────────────────────────────

today = date.today()
cutoff_date = today - relativedelta(months=TRAINING_LOOKBACK_MONTHS)

# Oct 1 of the FY that contains cutoff_date
if cutoff_date.month >= 10:
    training_start = date(cutoff_date.year, 10, 1)
else:
    training_start = date(cutoff_date.year - 1, 10, 1)

training_end = training_start + relativedelta(months=TRAINING_WINDOW_MONTHS)

print(f"Today                   : {today}")
print(f"{TRAINING_LOOKBACK_MONTHS} months ago           : {cutoff_date}")
print(f"FY start (>= {TRAINING_LOOKBACK_MONTHS}mo old) : {training_start}")
print(f"FY start + {TRAINING_WINDOW_MONTHS} months    : {training_end}")
...
SELECT * 
FROM {TRAINING_TABLE}
LEFT JOIN trm_reporting.silver.milestone m 
ON {TRAINING_TABLE}.serial_number = m.ser_num
WHERE
  serial_number IS NOT NULL
  AND disposal_nul_flag != 1
  AND m.filing_dt >= '{training_start}'
  AND m.filing_dt <= '{training_end}'
""")
...
display(sweep_df.sort_values('val_recall', ascending=False).head(10))
...
if opt_reg_threshold_90r is None:
    ...
    print('\nTop 10 configurations by precision (recall >= 90%):')
    display(sweep_df_90r.sort_values('val_precision', ascending=False).head(10))
...
if opt_reg_threshold_95r is None:
    ...
    print('\nTop 10 configurations by precision (recall >= 95%):')
    display(sweep_df_95r.sort_values('val_precision', ascending=False).head(10))
